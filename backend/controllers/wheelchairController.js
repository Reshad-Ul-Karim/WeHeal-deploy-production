import WheelchairOrder from '../models/wheelchairOrderModel.js';
import { User } from '../models/userModel.js';

// Create a new wheelchair order
export const createOrder = async (req, res) => {
  try {
    console.log('=== Creating wheelchair order ===');
    ...
    // Create the order
    const order = new WheelchairOrder({
      userId,
      planId,
      deliveryType,
      totalAmount,
    ...
    console.error('=== Error creating wheelchair order ===');
...
export const processPayment = async (req, res) => {
  try {
    ...
    // Find the order
    const order = await WheelchairOrder.findById(orderId);
    if (!order) {
...
export const getUserOrders = async (req, res) => {
  try {
    ...
    const orders = await WheelchairOrder.find({ userId })
      .sort({ createdAt: -1 })
      .limit(limit * 1)
      .skip((page - 1) * limit)
      .populate('deliveredBy', 'name');

    const total = await WheelchairOrder.countDocuments({ userId });
...
export const getAllOrders = async (req, res) => {
  try {
    ...
    const orders = await WheelchairOrder.find(filter)
      .sort({ createdAt: -1 })
      .limit(limit * 1)
      .skip((page - 1) * limit)
      .populate('userId', 'name email phone')
    ...
    const total = await WheelchairOrder.countDocuments(filter);
...
export const getOrderStats = async (req, res) => {
  try {
    const stats = await WheelchairOrder.aggregate([
      {
        $group: {
          _id: '$status',
          count: { $sum: 1 },
    ...
    const totalOrders = await WheelchairOrder.countDocuments();
    const totalRevenue = await WheelchairOrder.aggregate([
      {
    ...
    const planStats = await WheelchairOrder.aggregate([
      {
        $group: {
          _id: '$plan.id',
          count: { $sum: 1 },
